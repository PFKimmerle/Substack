...
class App(tk.Tk):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        for config_dict in pets_configs_list:
            ...
            path = config_dict["abs_path"]
            if not os.path.isabs(path):
                abs_path = os.path.join(ROOT_DIR_PATH, path)
            else:
                abs_path = path
    ...
    def create_pet(self, key) -> None:
        ...
        self.loaded_pets_dict[pet_name] = {
            ...
            "pet": Pet(canvas=self.canvas,
                       ...
                       playback_speeds_list=config.playback_speeds)
        }
...
class PetCreator(tk.Toplevel):
    ...
    def submit(self) -> None:
        ...
        # move the 2 special actions to the end of the list
        sub_list = []
...
class Pet:
    def __init__(self, canvas, pet_actions_full_path, sequence_list, weights_list, speeds_list, playback_speeds_list) -> None:
        ...
        self.playback_speeds_list = playback_speeds_list
        ...
        self.event4 = self.canvas.tag_bind(self.image_container,
                                           '<Button-3>', self.dance_pet)  # right-click for dance
    ...
    def process(self) -> bool:
        try:
            actions_names = sorted([gif for gif in os.listdir(self.actions_full_path)
    ...
    def update_frame(self) -> None:
        if self.is_drag:
            self.action_idx = 7  # dragging action (fixed index)
            self.cycle = 0
            return

        if self.falling:
            self.action_idx = 6  # falling action (fixed index)
            # Keep cycle at fall_toggle value (set in release_pet), don't reset to 0
            return

        # If we just landed from falling, go straight to idle (don't animate through fall frames)
        if self.action_idx == 6:  # was falling action (fixed index)
            self.cycle = 0
            self.action_idx = 0  # idle
            return

        # For glitch actions: decide how many frames to play this loop
        total_frames = len(self.actions_img_objs[self.action_idx])
        if self.action_idx in self.glitch_actions:
            count = self.loop_counter.get(self.action_idx, 0)
            if count % self.glitch_every_n != (self.glitch_every_n - 1):
                total_frames -= 1  # skip glitch frame (last frame)

        if self.cycle < total_frames - 1:
            self.cycle += 1
        else:  # end of an action
            if self.action_idx in self.glitch_actions:
                self.loop_counter[self.action_idx] = self.loop_counter.get(self.action_idx, 0) + 1
            self.cycle = 0
    ...
    def draw_loop(self) -> None:
        ...
        edge_margin = 5
        right_edge = self.screen_width - self.pet_width
        bottom_edge = self.screen_height - self.pet_height

        # PERIMETER WALK TRANSITIONS
        # Walk right hits right edge -> climb up
        if self.action_idx == 5 and self.x >= right_edge - edge_margin:
            self.x = right_edge
            self.action_idx = 9  # crawl_right_up
            self.cycle = 0
            x_dist = 0

        # Walk left hits left edge -> climb up
        elif self.action_idx == 4 and self.x <= edge_margin:
            self.x = 0
            self.action_idx = 11  # crawl_left_up
            self.cycle = 0
            x_dist = 0

        # Crawl right up hits top -> 15% fall or go left on top
        elif self.action_idx == 9 and self.y <= edge_margin:
            self.y = 0
            if random.random() < 0.15:
                self.falling = True
                self.action_idx = 6
                self.cycle = self.fall_toggle
                self.fall_toggle = 1 - self.fall_toggle
            else:
                self.action_idx = 14  # crawl_top_left
                self.cycle = 0
            y_dist = 0

        # Crawl left up hits top -> 15% fall or go right on top
        elif self.action_idx == 11 and self.y <= edge_margin:
            self.y = 0
            if random.random() < 0.15:
                self.falling = True
                self.action_idx = 6
                ...
                self.action_idx = 13  # crawl_top_right
                self.cycle = 0
            y_dist = 0

        # Crawl top right hits right corner -> 15% fall or climb down
        elif self.action_idx == 13 and self.x >= right_edge - edge_margin:
            self.x = right_edge
            if random.random() < 0.15:
                self.falling = True
                self.action_idx = 6
                ...
                self.action_idx = 10  # crawl_right_down
                self.cycle = 0
            x_dist = 0

        # Crawl top left hits left corner -> 15% fall or climb down
        elif self.action_idx == 14 and self.x <= edge_margin:
            self.x = 0
            if random.random() < 0.15:
                self.falling = True
                self.action_idx = 6
                ...
                self.action_idx = 12  # crawl_left_down
                self.cycle = 0
            x_dist = 0

        # Crawl right down hits bottom -> walk
        elif self.action_idx == 10 and self.y >= bottom_edge - edge_margin:
            self.y = bottom_edge
            self.action_idx = 4  # walk_left
            self.cycle = 0
            y_dist = 0

        # Crawl left down hits bottom -> walk
        elif self.action_idx == 12 and self.y >= bottom_edge - edge_margin:
            self.y = bottom_edge
            self.action_idx = 5  # walk_right
            self.cycle = 0
            y_dist = 0

        # Prevent going off screen
        if self.x + x_dist > right_edge:
            x_dist = right_edge - self.x
        if self.x + x_dist < 0:
            x_dist = -self.x
        if self.y + y_dist < 0:
            y_dist = -self.y
        if self.y + y_dist > bottom_edge:
            y_dist = bottom_edge - self.y

        # Handle falling landing
        if self.y >= bottom_edge and not self.is_drag:
            self.falling = False
        ...
        self.after_id = self.canvas.after(self.playback_speeds_list[self.action_idx], self.draw_loop)
    ...
    def release_pet(self, event) -> None:
        self.falling = True
        self.is_drag = False
        # Set falling action immediately (fixed index)
        self.action_idx = 6  # falling action
        # Alternate fall frame each time
        self.cycle = self.fall_toggle
        self.fall_toggle = 1 - self.fall_toggle  # toggle between 0 and 1

    def dance_pet(self, event) -> None:
        # Double-click triggers dance (action index 8)
        self.is_drag = False
        self.falling = False
        self.action_idx = 8  # dance action
        self.cycle = 0
